...
object DynamicCaissonCompiler {
    def main(args: Array[String]) {
       ...
       try {
          ...
          primitiveAST.setCompilerInternals
          val transformedAST = primitiveAST.dynamicCheckTransform
          val writer = new java.io.FileWriter(outputFileName)
          writer.write(transformedAST.genCode)
          //the validation rules might need to be changed
          ...
          //TODO: need to add dynamic checks and statically guarantee upgrading of control dependent variables
          //the code generation phase will have to be rewritten
          //val writer = new java.io.FileWriter(outputFileName)
...
//    private def discard(x: AnyRef) {}
    ...
    private def printHelpAndExit(): (String, String) = {
      println("Usage: fsc DynamicCaissonCompiler <caissonFileName>  [-o <outputFileName>]")
      exit(-1)
    }
